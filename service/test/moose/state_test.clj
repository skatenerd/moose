...
(deftest
  owner-test_0
  (testing-both-implementations
    "it knows the owner after single add"
    (add-requestor "abc" "karl marx")
    (is (= "karl marx" (owner "abc")))))
...
(deftest
  tokens-and-subscriptions-test
  (testing-both-implementations
    "lists tokens held and waited for by user"
    (add-requestor "tdd" "ronald")
    (add-requestor "tdd" "kent")
    (add-requestor "waterfall" "kent")
    (is (= {:held '(waterfall)
            :waiting {"tdd" 1}
            }
           (tokens-and-subscriptions "kent")))
